<?php 

//error_reporting(E_ALL);

$host='//'.$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']);



$server="//www.oficloud.com/client";


set_include_path($_SERVER["DOCUMENT_ROOT"].'/phpseclib');

define('CRYPT_RSA_EXPONENT', 65537);

$error="";

if(isset($_REQUEST["privk1"]))

{

	include_once('Crypt/RSA.php');
	include_once('Crypt/AES.php');
	include_once('Crypt/Random.php');

	include_once("../db.php");

	

	$con = new mysqli($g_db_server, $g_db_user, $g_db_password,$g_db_name) or $error=-1;

	

	

	$token=$con->real_escape_string($_REQUEST["token"]);
	$privk1=$con->real_escape_string($_REQUEST["privk1"]);
	$privk2=$con->real_escape_string($_REQUEST["privk2"]);
	$pubk1=$con->real_escape_string($_REQUEST["pubk1"]);
	$pubk2=$con->real_escape_string($_REQUEST["pubk2"]);

	//$host=$con->real_escape_string($_REQUEST["host"]);

function arreglar_clave($k)
{
	/*$res=str_replace('\n',"\\n",$k);
	$res=str_replace("\n","\\n",$res);
	$res=str_replace('\r',"",$res);
	$res=str_replace("\r","",$res);*/

	return $k;
}

	$privk1=arreglar_clave($privk1);
	$privk2=arreglar_clave($privk2);
	$pubk1=arreglar_clave($pubk1);
	$pubk2=arreglar_clave($pubk2);

	//$con->query("START TRANSACTION");
	//$con->autocommit(FALSE);

	/*$sql="SELECT privkey,privkey_signing,pubkey,pubkey_signing FROM config";
	$res=$con->query($sql);
	$row=$res->fetch_assoc();
	$privkey=$row["privkey"];
	$privkey_signing=$row["privkey_signing"];
	$pubkey_signing=$row["pubkey_signing"];*/

	

	$sql="TRUNCATE TABLE config";
	$con->query($sql);

	$sql="INSERT INTO config(privkey,pubkey,privkey_signing,pubkey_signing,tokenkey) VALUES('$privk1','$pubk1','$privk2','$pubk2','$token')";
	//echo "$sql";
	$con->query($sql);

	//$con->query("COMMIT");
	

	$postdata = http_build_query(
			array(
					'token' => $token
			)
			);

	$opts = array('http' =>
			array(
					'method'  => 'POST',
					'header'  => 'Content-type: application/x-www-form-urlencoded',
					'content' => $postdata
			)
	);


	$prevserver=$server;

	$context  = stream_context_create($opts);

	@$respuesta= file_get_contents("https:$server/get_keysharing_server.php", false, $context);
	if($respuesta===FALSE)
	{
		$error="The Oficloud server ($server) is not responding. Please try later or check your Internet connection.";
	}
	else 
	{
		//echo $respuesta;
		
		$p=json_decode($respuesta);
		if($p->e=="OK")
		{	
			if($p->keyserver)
				$prevserver=$p->keyserver;
			
			$certB=$host;
			$rsa = new Crypt_RSA();

			$rsa->loadKey(str_replace("\r","",str_replace("\n","",$privk2))); // private key
			$rsa->setHash("sha256");
			$rsa->setSignatureMode(CRYPT_RSA_SIGNATURE_PKCS1);
			$signature2 = $rsa->sign($certB);

			$postdata = http_build_query(
				array(
					'token' => $token,
					'host' => $host,
					'signature' => $signature2
				)
			);

			$opts = array('http' =>
				array(
					'method'  => 'POST',
					'header'  => 'Content-type: application/x-www-form-urlencoded',
					'content' => $postdata
				)
			);

			$context  = stream_context_create($opts);
			
			//echo "$prevserver";
			$respuesta= file_get_contents("http:$prevserver/get_keysharing_data.php", false, $context);
			if($respuesta===FALSE)
			{
				$error="The Oficloud server ($prevserver) is not responding. Please try later or check your Internet connection.";
			}
			else 
			{
				//echo $respuesta;
				$p=json_decode($respuesta);
				if($p && $p->e=="OK")
				{	
					$mysqli = new mysqli($g_db_server, $g_db_user, $g_db_password, $g_db_name);
					foreach($p->dump as $query)
					{
						$query= str_replace("\\n","\r\n",urldecode($query));
						//echo "$query<br>";
						if ($mysqli->multi_query($query)) {
							$b=true;
							do {
								// almacenar primer juego de resultados 
								if ($result = $mysqli->store_result()) {
									while ($row = $result->fetch_row()) {

									}
									$result->free();
								}
								// mostrar divisor 
								if ($mysqli->more_results()) {
									$mysqli->next_result();
								}
								else
									$b=false;
							} while ($b);
						}
					}
					/* cerrar conexi�n */
					$mysqli->close();

					$postdata = http_build_query(
						array(
							'token' => $token,
							'pubk1' => $pubk1,
							'pubk2' => $pubk2,
							'host' => substr($host,0,-8)
						)
					);
					$opts = array('http' =>
						array(
								'method'  => 'POST',
								'header'  => 'Content-type: application/x-www-form-urlencoded',
								'content' => $postdata
						)
					);

					$context  = stream_context_create($opts);

					@$respuesta= file_get_contents("https:$server/set_keysharing_server.php", false, $context);

					if($respuesta===FALSE)
					{
						$error="The Oficloud server ($server) is not responding. Please try later or check your Internet connection.";
					}
					else 
					{
						//echo $respuesta;
						$p=json_decode($respuesta);
						if($p->e=="OK")
						{	
							//$con->query("COMMIT");
							/*if (!$mysqli->commit()) {
    							$error="COMMIT failed";
    							exit();
							}*/
							header("Location:confirmed.php");
							die;
						}
						else 
						{
							echo "error 3: ".$respuesta;
							$error=$p->e;
						}
					}

				}
				else 
				{
					echo "error 2: ".$respuesta." from $prevserver";
					$error=$p->e;
				}
			}
		}
		else
		{
			echo "error 1: ".$p->message;
			$error=$p->e;
		}
	}



}


{
	include_once('Crypt/RSA.php');
	include_once('Crypt/AES.php');
	include_once('Crypt/Random.php');

	include_once("../db.php");

	

	$rsa = new Crypt_RSA();
	$keypair = $rsa->createKey(2048);
	$keypair2 = $rsa->createKey(2048);
}



?>

<!DOCTYPE html>

<html lang="en">

<head>



    <meta charset="utf-8">

    <meta name="viewport" content="width=device-width, initial-scale=1">

    <meta name="description" content="">

    <meta name="author" content="">



    <title>

    	Oficloud Installer

    </title>



    <!-- Bootstrap Core CSS -->

    <link href="/css/bootstrap.min.css" rel="stylesheet">



<link href="css/estilo.css" rel="stylesheet">



<style>

.myinput

{

	width:100%;

	height:80px;

}

</style>



</head>

<body>

<div id="mainwindow" class="ventana">

<div class="titulo">Oficloud installer</div>

<div class="contenido">

	<form id="miform" action="" method="POST">

	<h2>Insert the token-key obtained from your account</h2>

	<input type="text" name="token" style="width:100%; margin:0px 0px;"/>

	<h2>Generated public keys</h2>

	Public key 1: <textarea type="text" name="pubk1" id="pk1" class="myinput" readonly><?=$keypair["publickey"]?></textarea><br>

	Public key 2: <textarea type="text" name="pubk2" id="pk2" class="myinput" readonly><?=$keypair2["publickey"]?></textarea><br>

	<input type="hidden" id="privk1" name="privk1" value="<?=$keypair["privatekey"]?>"/>

	<input type="hidden" id="privk2" name="privk2" value="<?=$keypair2["privatekey"]?>"/>

	<input type="hidden" name="host" value="<?=$host?>"/>

	</form>

	<span style="color:#de0000"><?=$error?></span>

</div>

<button class="btn" style="float: left;" onclick="location.href='database.php'">&lt;&lt;&lt;Previous</button>

<button class="btn" onclick="next();">Continue&gt;&gt;&gt;</button>

</div>



<script>

	function next()

	{		

		var e=document.getElementById('miform');

		e.submit();

	}

</script>





</script>



</body>

</html>