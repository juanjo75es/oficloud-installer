...
class MySQLDump {
  ...
  /**
	...
	* @return boolean
	*/
	function getDatabaseStructure($jjtabla){
		...
		//return true;
        return $structure;*/
		return ($this->getTableStructure($jjtabla));
  }
	/**
	* Writes to file all the selected database tables data
	* @param boolean $hexValue It defines if the output is base-16 or not
	*/
	function getDatabaseData($jjtabla, $hexValue = true){
		/*$records = @mysql_query('SHOW TABLES');
		if ( @mysql_num_rows($records) == 0 )
			return false;
        $data="";
		...
		$adata=$this->getTableData($jjtabla,$hexValue);
        return $adata;
  }
	/**
	* Writes to file the selected database dump
	*/
	function doDump($jjtabla, $params = array(), $close_file = true) {
		//$this->saveToFile($this->file,"SET FOREIGN_KEY_CHECKS = 0;\n\n");
        $res=array("SET FOREIGN_KEY_CHECKS = 0;\n\n");
		if (!isset($params['skip_structure']))
			$res=array_merge($res,$this->getDatabaseStructure($jjtabla));
		if (!isset($params['skip_data']))
			$res=array_merge($res,$this->getDatabaseData($jjtabla,$this->hexValue));
